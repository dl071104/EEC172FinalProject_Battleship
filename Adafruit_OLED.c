...
*/
    unsigned long f;
    GPIOPinWrite(GPIOA3_BASE, 0x10, 0); //DC  0 = command
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0); //CS low
    SPICSEnable(GSPI_BASE);
    SPIDataPut(GSPI_BASE, c);
    SPIDataGet(GSPI_BASE, &f);
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40); //CS high
...
*  SPI.
*/  unsigned long f;
    GPIOPinWrite(GPIOA3_BASE, 0x10, 0x10); //DC 1 = data
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0); //CS low
    SPICSEnable(GSPI_BASE);
    SPIDataPut(GSPI_BASE, c);
    SPIDataGet(GSPI_BASE, &f);
